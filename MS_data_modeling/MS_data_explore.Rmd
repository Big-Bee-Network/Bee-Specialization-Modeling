...
compare_df <- merge(compare_df, globee_formatGenus, by = c('interestBee', 'number_of_citation_for_bee_species'), all = T)  
compare_df <- merge(compare_df, globee_5cut_formatGenus, by = c('interestBee'), all = T) 
compare_df <- merge(compare_df, globee_T_formatGenus, by = c('interestBee'), all = T) 
...
final <- compare_df %>% filter(existsMS == 1, existsGLOBEE == 1) %>% 
  dplyr::rename(interest_bee_species = interestBee, number_of_citations_for_bee_species_GloBi = number_of_citation_for_bee_species, degree_raw_GloBi_family = degreeGLOBEEFamily,
                pollen_hosts_GloBi_family = pollenHostGLOBEEFamily, pollen_host_genus_MS = pollenHostsMS, degree_MS_family = degreeMS, degree_MS_Genus = degreeMSGenus,
                ...
                degree_raw_GloBi_genus = degreeGLOBEEGenus, pollen_hosts_GloBi_genus = pollenHostGLOBEEGenus, degree_cut_5_GloBi_genus = degreeGLOBEE_C5Genus,
                degree_transformed_GloBi_genus = degreeGLOBEE_TGenus, specialist_FOWLER = specialistFOWLER) %>% 
  dplyr::select(interest_bee_species, number_of_citations_for_bee_species_GloBi, degree_raw_GloBi_family, degree_cut_5_GloBi_family, degree_transformed_GloBi_family,
                pollen_hosts_GloBi_family, degree_MS_family, pollen_host_family_MS, degree_raw_GloBi_genus, degree_cut_5_GloBi_genus, degree_transformed_GloBi_genus,
                pollen_hosts_GloBi_genus, pollen_host_genus_MS, specialization_MS) %>% 
           mutate(pollen_host_family_MS = str_remove(pollen_hosts_GloBi_family, "NA, "),
                  pollen_hosts_GloBi_family = str_to_title(pollen_hosts_GloBi_family), pollen_host_family_MS =str_to_title(pollen_host_family_MS),
                  pollen_hosts_GloBi_genus = str_to_title(pollen_hosts_GloBi_genus))

final

write.csv(final, '/Users/nickbachelder/Desktop/Bee-Specialization-Modeling/modeling_data/specialization_degree_df')
```
