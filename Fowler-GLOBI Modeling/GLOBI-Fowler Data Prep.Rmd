...
interactions_b <- read.csv('/Users/nickbachelder/Desktop/Bee-Specialization-Modeling/interactions_dup.csv') %>% dplyr::select(-X)
...
beeCiteNoFilter <- interactions_b %>% dplyr::select(sourceTaxonSpeciesName,targetTaxonFamilyName) %>% 
...
write.csv(plotdf, 'BeeCiteRawGenus')
```
...
write.csv(plotdf, 'BeeCiteC5Genus')
```
...
write.csv(plotdf, 'BeeCiteWGenus')
```
