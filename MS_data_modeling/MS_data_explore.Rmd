...
compare_df <- compare_df %>% merge(fowler %>% rename(specialistFOWLER = specialist) %>% 
  ...
  rename(pollenHostsMS = pollenHosts,ReferencesMS = References) %>%
  select(interestBee, specializationMS, degreeGLOBEE, degreeGLOBEE_C5, degreeGLOBEE_T, specialistFOWLER, 
         existsGLOBEE, existsMS, existsFOWLER, existsALL, pollenHostsMS, pollenHostGLOBEE, ReferencesMS, degreeMS)
 
compare_df %>% filter(existsALL == 0)
```
...
compare_df %>% filter(specialistFOWLER == 1, specializationMS  == 'Polylectic') %>% 
  select(interestBee, specializationMS, specialistFOWLER, degreeGLOBEE_C5, degreeGLOBEE, pollenHostsMS, pollenHostGLOBEE, ReferencesMS) %>% filter(degreeGLOBEE < 3)
```
...
compare_df %>% filter(specialistFOWLER == 0, specializationMS  != 'Polylectic') %>% 
  select(interestBee, specializationMS, specialistFOWLER, degreeGLOBEE_C5, degreeGLOBEE, pollenHostsMS, pollenHostGLOBEE, ReferencesMS) %>% filter(degreeGLOBEE_C5 > 5)
```
