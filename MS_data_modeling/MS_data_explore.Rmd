...
MS_data_format <- MS_data %>% mutate(interestBee = str_to_title(paste(Genus, sub(".*? ", "", Species)))) %>% 
  rename(degreeMS = 'Pollen host family #', specializationMS = 'Redefined pollen diet breadth', pollenHosts = 'Pollen hosts') %>%
  select(interestBee, pollenHosts, degreeMS, specializationMS, References) %>%
  filter(!is.na(degreeMS)) %>% arrange(interestBee) %>% mutate(pollenHosts = str_to_title(gsub("[A-Za-z]*[^A-Za-z/, ]\\S*\\s*", "", pollenHosts)))
...
pollenHostGLOBEE <- aggregate(interact~interestBee, globee_5cut, FUN=toString) %>% mutate(interestBee = str_to_title(interestBee)) %>% 
  rename('pollenHostGLOBEE' = interact)
...
compare_df <- compare_df %>% merge(fowler %>% rename(specialistFOWLER = specialist) %>% 
  ...
  mutate(existsGLOBEE = ifelse(is.na(degreeGLOBEE), 0, 1), existsMS = ifelse(is.na(degreeMS), 0, 1), 
         ...
         specialistFOWLER = ifelse(is.na(specialistFOWLER), 0 , specialistFOWLER),
         pollenHostGLOBEE = str_to_title(pollenHostGLOBEE)) %>% rename(pollenHostsMS = pollenHosts,
                                                                                              ReferencesMS = References) %>%
  select(interestBee, specializationMS, degreeGLOBEE, degreeGLOBEE_C5, degreeGLOBEE_T, specialistFOWLER, 
         existsGLOBEE, existsMS, existsFOWLER, existsALL, pollenHostsMS, pollenHostGLOBEE, ReferencesMS)
...
compare_df %>% filter(specialistFOWLER == 1, specializationMS  == 'Polylectic') %>% 
  select(interestBee, specializationMS, specialistFOWLER, degreeGLOBEE_C5, degreeGLOBEE, pollenHosts) %>% filter(degreeGLOBEE_C5 > 3)
```
...
compare_df %>% filter(specialistFOWLER == 0, specializationMS  != 'Polylectic') %>% 
  select(interestBee, specializationMS, specialistFOWLER, degreeGLOBEE_C5, degreeGLOBEE) %>% filter(degreeGLOBEE_C5 > 5)
```

## Plan: want to add pollen hosts so I can filter on pollen family. Discuss how to parse the authors out of appropriate MS column. ## DONE
...
## Check how many GLOBI bees were cut when filtering for fowler, add them back (update previously done tables). ## DONE: Turns out that we didnt cut globi bees, but we instead cut FOWLER bees.
